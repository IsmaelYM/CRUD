...
public class Conexion {
	...
	public void rellenarListadoProveedor(TextArea txaListadoProveedor)
	{
		...
		try
		{
			...
			// y ejecutar la sentencia SQL
			ResultSet resultado = statement.executeQuery(sentencia);
			apunteLog("Consulta Compra: "+sentencia);
			while(resultado.next())
			{
	...
	public void rellenarListadoFactura(TextArea txaListado) {
	    String sentencia = "SELECT idCompra, fechaCompra, cantidadCompra, nombreCliente, primerApellidoCliente, nombreProducto, precioProducto, (concat(cantidadCompra * precioProducto, ' €')) AS Total FROM comprar JOIN cliente ON comprar.idClienteFK = cliente.idCliente JOIN producto ON comprar.idProductoFK = producto.idProducto;";
	    try {
	        // Create a statement
